...
function observe_images(el: HTMLElement): void {
    const observer = new IntersectionObserver((entries: IntersectionObserverEntry[], observer: IntersectionObserver) => {
        ...
        if (entries[0].isIntersecting) {
            (entries[0].target as HTMLImageElement).srcset = (entries[0].target as HTMLImageElement).dataset.srcset!;
...
export default {
    ...
    unmounted(): void {
        window.removeEventListener("streaming-list-update", () => observe_images);
